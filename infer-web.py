...
import os
from evals import check_similarity, generate_wer
import soundfile
...
def generate_output(spk_item, model, input_path, filename):

  model_dir = model.split(".")[0]
  audio_data = vc_single(sid = spk_item,
                      input_audio = input_path,
                      f0_up_key = 0.0,
                      f0_file = None,
                      f0_method = "pm",
                      file_index = f"./logs/{model_dir}/added_IVF4_Flat_nprobe_1.index",
                      index_rate = 0.76,)[1][1]

  output_path = f"./evals/output/{filename}"
  soundfile.write(output_path, audio_data, 44100)

  return output_path
    

def evaluate_model(spk_item, model):
  model_dir = model.split(".")[0]
  similarity = 0
  intel = 0
  for filename in os.listdir("./evals/input"):
    if any(filename.endswith(e) for e in ["mp3", "wav"]):
      input_path = "./evals/input/" + filename
      output_path = generate_output(spk_item, model, input_path, filename)

      if filename.startswith("similarity"):
        dataset_path = os.listdir(f"./logs/{model_dir}/1_16k_wavs")[0]
        similarity = check_similarity(input_path, output_path)
        print("similarity: ", similarity)

      elif filename.startswith("tts"):
        transcript = "Hello everyone, this is a small attempt by Dubverse, and audio generative AI company to add evals to this amazing voice cloning repository."
        intel = generate_wer(output_path, transcript)
        print("intel: ", intel)
      else:
        continue
      
  return (intel, similarity)
...
with gr.Blocks() as app:
    ...
    with gr.Tabs():
        with gr.TabItem(i18n("Check Speaker Similarity")):
            with gr.Row():
                model = gr.Textbox(label = i18n("Write Model Name Here (with the .pth extension)"), interactive = True)
            ...
            with gr.Row():
                intelligibility = gr.Textbox(label = i18n("Intelligibility"), interactive = False)
                similarity = gr.Textbox(label = i18n("Similarity"), interactive = False)
                evaluate_button = gr.Button(i18n("Evaluate Models"), variant="primary")
                evaluate_button.click(fn = evaluate_model, inputs = [spk_item, model], outputs = [intelligibility, similarity])
