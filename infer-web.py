...
def load_speech_model(speech_encoder="hubert_base"):
    global speech_model
    if speech_encoder == "hubert_base":
        ...
        speech_model = speech_model.to(config.device)
        if config.is_half:
            speech_model = speech_model.half()
        else:
            speech_model = speech_model.float()
        speech_model.eval()
    elif speech_encoder == "PPGLarge":
        from whisperPPGLarge import WhisperPPGLarge
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        speech_model = WhisperPPGLarge(device=device)
...
def vc_single(
    ...
    global tgt_sr, net_g, vc, speech_model, version
    ...
    try:
        ...
        times = [0, 0, 0]
        # if not speech_model:
        load_speech_model(speech_encoder)
        if_f0 = cpt.get("f0", 1)
        file_index = (
            (
                file_index.strip(" ")
...
# 一个选项卡全局只能有一个音色
def get_vc(sid, to_return_protect0, to_return_protect1, speech_encoder):
    global n_spk, tgt_sr, net_g, vc, cpt, version
    if speech_encoder == "PPGLarge":
        enc_out_dim = 1280
    elif speech_encoder == "hubert_base":
        enc_out_dim = 768
    if sid == "" or sid == []:
        if speech_model is not None:  # 考虑到轮询, 需要加个判断看是否 sid 是由有模型切换到无模型的
            print("clean_empty_cache")
            del net_g, n_spk, vc, speech_model, tgt_sr  # ,cpt
            speech_model = net_g = n_spk = vc = speech_model = tgt_sr = None
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
            ###楼下不这么折腾清理不干净
            if_f0 = cpt.get("f0", 1)
            ...
            if version == "v1":
                if if_f0 == 1:
                    net_g = SynthesizerTrnMs256NSFsid(
                        *cpt["config"], is_half=config.is_half, enc_out_dim=256 if enc_out_dim == 768 else enc_out_dim
                    )
                else:
                    net_g = SynthesizerTrnMs256NSFsid_nono(*cpt["config"], enc_out_dim=256 if enc_out_dim == 768 else enc_out_dim)
            elif version == "v2":
                if if_f0 == 1:
                    net_g = SynthesizerTrnMs768NSFsid(
                        *cpt["config"], is_half=config.is_half, enc_out_dim=enc_out_dim
                    )
                else:
                    net_g = SynthesizerTrnMs768NSFsid_nono(*cpt["config"], enc_out_dim=enc_out_dim)
            del net_g, cpt
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
            cpt = None
    ...
    if version == "v1":
        if if_f0 == 1:
            net_g = SynthesizerTrnMs256NSFsid(*cpt["config"], is_half=config.is_half, enc_out_dim=256 if enc_out_dim == 768 else enc_out_dim)
        else:
            net_g = SynthesizerTrnMs256NSFsid_nono(*cpt["config"], enc_out_dim=256 if enc_out_dim == 768 else enc_out_dim)
    elif version == "v2":
        if if_f0 == 1:
            net_g = SynthesizerTrnMs768NSFsid(*cpt["config"], is_half=config.is_half, enc_out_dim=enc_out_dim)
        else:
            net_g = SynthesizerTrnMs768NSFsid_nono(*cpt["config"], enc_out_dim=enc_out_dim)
    del net_g.enc_q
...
with gr.Blocks() as app:
    ...
    with gr.Tabs():
        with gr.TabItem(i18n("模型推理")):
            ...
            sid0.change(
                fn=get_vc,
                inputs=[sid0, protect0, protect1, speech_encoder],
