...
def train_index(exp_dir1):
    ...
    yield "\n".join(infos)
    index.train(big_npy)
    faiss.write_index(index, '%s/trained_IVF%s_FastScan.index'%(exp_dir,n_ivf))
    infos.append("adding")
    yield "\n".join(infos)
    index.add(big_npy)
    faiss.write_index(index, '%s/added_IVF%s_FastScan.index'%(exp_dir,n_ivf))
    infos.append("成功构建索引，added_IVF%s_FastScan.index"%(n_ivf))
    yield "\n".join(infos)
...
def train1key(exp_dir1, sr2, if_f0_3, trainset_dir4, spk_id5, gpus6, np7, f0method8, save_epoch10, total_epoch11, batch_size12, if_save_latest13, pretrained_G14, pretrained_D15, gpus16, if_cache_gpu17):
    ...
    index.add(big_npy)
    faiss.write_index(index, '%s/added_IVF%s_FastScan.index'%(exp_dir,n_ivf))
    yield get_info_str("成功构建索引，added_IVF%s_FastScan.index"%(n_ivf))
