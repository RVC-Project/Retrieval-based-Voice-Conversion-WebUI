...
async def auto_tts():
    ...
    rate = '+0%'
    volume = '+0%'
    print("TTS:"+input_text)
    tts = edge_tts.Communicate(text=input_text, voice=voice, rate=rate, volume=volume)
    await tts.save("./tmp.mp3")
    print("TTS finished.")
...
def tts_warp(input):
    ...
    event_loop.run_until_complete(auto_tts())
    event_loop.close()
...
def vc_single(
    ...
    if input_audio is None:
        return "TTS failed or no text inputed", None
    f0_up_key = int(f0_up_key)
    tts_warp(input_audio)
    try:
        audio = load_audio("./tmp.mp3", 16000)
        times = [0, 0, 0]
