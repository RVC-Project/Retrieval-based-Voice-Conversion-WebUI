...
> **ステータス**: M0/M1実装完了（2026-04-11更新）。セグメント長拡張（48k=34560, 32k=25600）およびbf16混合精度はM1で実装済み。TextEncoder拡張、LoRA、Causal Convolutionは未実装（M4以降）。
...
### 1. セグメント長拡張 ✅ Step 1 実装済み（M1: 48k=34560, 32k=25600）
...
| Step 1 | 34,560 | 0.72s | 72 | 5-7 | ✅ 実装済み（M1） |
...
|------|-------------------|------|-----------|
| 現行 | 12,800 | 0.40s | 40 |
| Step 1 | 25,600 | 0.80s | 80 | ✅ 実装済み（M1） |
...
### Phase 1: セグメント長拡張のみ（最小変更） ✅ 実装済み（M1）
...
### ステップ1（Week 1）: セグメント長拡張 + bfloat16 ✅ 実装済み（M1）
...
> **M1実装完了（2026-04-11）**: セグメント長拡張（48k=34560, 32k=25600）およびbf16混合精度はM1マイルストーンで実装済み。Phase 1, 2が完了している。
