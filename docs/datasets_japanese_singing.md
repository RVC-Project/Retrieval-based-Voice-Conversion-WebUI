...
| 項目 | 詳細 |
|------|------|
| URL | https://sites.google.com/site/shinnosuketakamichi/research-topics/pjs_corpus |
| 歌唱者 | 1名 |
| 曲数 | 100曲（短い歌唱）+ 100文（読み上げ） |
| サンプリングレート | **48kHz** |
| 形式 | アカペラ、UVR5分離不要 |
| ジャンル | 音素バランス考慮オリジナル楽曲 |
| ライセンス | **CC BY-SA 4.0（商用利用可）** |
| 品質 | 静音環境録音 |
| RVC適性 | **★★★★★** 48kHz + 商用ライセンス + 音素バランス保証 |

**推奨用途**: 単一話者の高品質モデル学習。音素バランスが保証された唯一の日本語歌声コーパス。
...
```
事前学習:
  NIT-SONG070 (CC BY 3.0) + PJS (CC BY-SA 4.0)
    ↓
ターゲット話者ファインチューニング:
  声優の歌声データ 10分
```

CC BY / CC BY-SA ライセンスのデータのみで構成可能。
...
HuBERT事前学習:
  rinna/japanese-hubert-base（ReazonSpeech 19,000h で学習済み）
    ↓
RVCモデル事前学習:
  JVS-MuSiC + NIT-SONG070 + PJS + きりたん + イタコ + GTSinger
    ↓
歌声適応:
...
| 2 | **PJS** | - | 1名 | 48k | CC BY-SA 4.0 | Yes | オリジナル | ★★★★★ |
