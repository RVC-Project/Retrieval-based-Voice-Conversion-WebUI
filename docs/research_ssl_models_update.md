...
>
> **ステータス**: 本調査レポートはSSLモデルの比較調査であり、コード実装を伴わない。SSLモデル統合はM2-A（未実装）として計画されている。M0で構築済みの評価基盤（Whisper CER, MCD, F0 RMSE）を用いて、SSLモデル置換時の品質を定量比較する準備は整っている。
...
> **M0実装済み**: Whisper CER（large-v3, delta_CER）、MCD-DTW（n_mels=40）、F0 RMSE（cents）の自動評価基盤がtools/eval/run_eval.pyとして構築済み。上記比較実験の客観指標はCLIから即座に実行可能。
